...
class Room:
    def __init__(self, level):
        ...
        self.max_height = (int)(
            self.window_height / self.room_blocksize
        )
        # for grid logic
...
def draw_tile(room, x, y):
    ...
    rect = pygame.Rect(
        x_draw, y_draw, room.room_blocksize, room.room_blocksize
    )
    # draw rect
...
def draw_blocks(room):
    ...
    for rect in room.block_list:
        ...
        rect = pygame.Rect(
            x_draw,
            y_draw,
            room.room_blocksize * w,
            room.room_blocksize * h,
        )
        # draw rect
...
def drawGrid(room):
    ...
    for x in range(0, room.window_width, room.room_blocksize):
        for y in range(0, room.window_height, room.room_blocksize):
            rect = pygame.Rect(
                x, y, room.room_blocksize, room.room_blocksize
            )
            pygame.draw.rect(SCREEN, WHITE, rect, 1)
...
def main(game_level):
    ...
    SCREEN = pygame.display.set_mode(
        (room.window_width, room.window_height)
    )
    SCREEN.fill(BLACK)
    ...
    while True:
        ...
        # check if done..
        if (
            len(room.clean_set) == room.max_tiles
            or move_count > max_cycles
        ):
            result_str = " SUCCESS!"
...
if __name__ == "__main__":
    ...
    game_level = 3  # OK to change this from easy to more complex
